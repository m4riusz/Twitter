...
export class Comments {
    ...
    async activate(params, config) {
        ...
        [this.tweet, this.comments] = await Promise.all([
            ...
            this.commentService.getOldestCommentsFromTweet(params.tweetId, this.page, Const.PAGE_SIZE)
        ]);
    }
    ...
    async nextCommentPage() {
        try {
            this.page = ++this.page;
            let nextCommentPage = await this.commentService.getOldestCommentsFromTweet(this.tweet.id, this.page, Const.PAGE_SIZE);
