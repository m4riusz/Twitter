...
public final class Query {
    ...
    public static final String SELECT_COMMENT_FROM_TWEET_ORDER_BY_VOTE_SIZE = " SELECT comment FROM Comment comment LEFT JOIN comment.votes cm_votes WHERE comment.tweet.id = ?1  GROUP BY comment.id, comment.createDate, comment.version, comment.banned, comment.deleted, comment.content, comment.owner, comment.tweet ORDER BY MAX(cm_votes.vote) ASC, COUNT(cm_votes) DESC, comment.createDate DESC";
